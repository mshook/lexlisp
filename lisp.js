...
var ax, cx, dx, depth, panic, fail, g;
var cHeap, cGets, cSets, cReads, cPrints;
var kEq, kCar, kCdr, kCond, kAtom, kCons, kQuote, kDefine, kLambda, kFunarg;
...
function Apply(f, x, a) {
  if (f < 0)      return Eval(Car(Cdr(Cdr(f))),
                             Pairlis(Car(Cdr(f)), x, Cdr(Cdr(Cdr(f)))));
...
ReadChar() {
  int b, c, t;
  static char *freeme;
  static char *line = "NIL T EQ CAR CDR ATOM COND CONS QUOTE DEFINE LAMBDA FUNARG ";
...
var code, index, output, funcall, M, Null;
...
function Reset() {
  ...
  // }
  Load("NIL T EQ CAR CDR ATOM COND CONS QUOTE DEFINE LAMBDA FUNARG ");
  LoadBuiltins()
}
